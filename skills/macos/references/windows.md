...
For advice on arranging content inside a window across any platform, refer to **Layout**; for guidance on arranging content in Apple Vision Pro space, consult **Spatial layout**. For developer resources, see [Windows](apple:SwiftUI/Windows).

### Best practices
- **Ensure your windows adapt fluidly to different sizes to support multiwindow and multitasking workflows.** Refer to **Layout** and [Multitasking](multitasking.md) for detailed guidance.
...
The window's *frame* resides above the body area and may contain window controls along with a **Toolbars**. Additionally, in uncommon scenarios, the window may feature a bottom bar—a component of the frame displayed beneath the body content.
