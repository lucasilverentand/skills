...
Examples: `git/committing`, `git/branching`, `documentation/writing-style`, `security/auditing-security`
...
Examples: `git` (parent routing to children), `documentation/developer-docs` (composes `writing-style` + `gathering-knowledge`), `git/cleaning-repo`
...
2. **Cross-skill references** — the skill points to another skill for a specific concern (e.g., "for tone → see `documentation/writing-style`")
...
Example: `git/committing` started as the commit-writing step inside `git/cleaning-repo`. Extracted, it became independently useful — any workflow that creates commits can reference it.
