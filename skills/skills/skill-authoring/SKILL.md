...
- What are you doing?
  ...
  - **Improving an existing skill** -> what needs work?
    ...
    - **Description is vague or undertriggering** -> see `references/description-optimization.md` "Writing effective descriptions" for principles, or run the full optimization workflow via "Optimizing a description" below
...
The `description` field determines whether Claude invokes a skill — it carries the entire burden of triggering. See `references/description-optimization.md` for the full workflow and query design guidance.

1. **Design 20 eval queries** — 8-10 should-trigger, 8-10 should-not-trigger. Should-trigger queries should vary phrasing, explicitness, detail, and complexity. Should-not-trigger queries should be near-misses (shared keywords but different intent), not obviously irrelevant. All queries should be realistic: include file paths, personal context, casual language, typos.
2. **Review with user** via the HTML template in `assets/eval_review.html`
3. **Run the optimization loop** via `scripts/run-loop.ts` (requires Claude Code CLI) — automatically splits train/test, evaluates trigger rates, proposes improvements with extended thinking, and selects the best iteration by held-out test score
4. **Apply the best description** to SKILL.md frontmatter, verify it's under 1024 characters, and spot-check with fresh queries
