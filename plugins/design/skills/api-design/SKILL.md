...
- If the API is for a new service, suggest writing ADRs (`design:write-adr`) for non-obvious choices when available
- If there's a diagram, make sure it matches (`design:c4-diagrams`) when available
...
|The API is part of a larger design|`design:write-design-doc` for the parent doc if installed|
|A choice deserves a permanent record|`design:write-adr` if installed|
|A diagram would make the flow clearer|`design:c4-diagrams` if installed|
