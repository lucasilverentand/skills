...
# Architecture
Turns requirements into a concrete system shape — components, boundaries, data flow, failure modes, and technology choices. This is the thinking skill: it's about decomposition and trade-offs, not about writing the document (that's `design:write-design-doc` when installed), modeling data (that's `data-modeling`), or designing APIs (that's `api-design`).
...
- What does the user need?
  ...
  - **User doesn't have clear requirements yet** → hand off to `intent:requirements` first when available
  - **User wants a picture** → hand off to `design:c4-diagrams` when available
  - **User wants to capture a specific decision** → hand off to `design:write-adr` when available
...
|Need a diagram|`design:c4-diagrams` if installed|
|Ready to write the doc|`design:write-design-doc` if installed|
|A decision deserves recording|`design:write-adr` if installed|
