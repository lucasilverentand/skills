...
This is the source of truth for the `planning` plugin's Linear planning workflows. Use it when changing plugin behavior, Linear usage, or the reusable planner structure.
...
Codex and Claude must consume the same authored skill source under `plugins/planning/skills/`; generated manifests and READMEs are packaging outputs. The `issue-authoring` skill owns platform-neutral issue authoring, routing metadata, relationship rules, complexity scoring, and thin destination adapters. Update this brief when a change alters purpose, issue behavior, scope boundaries, or maintenance rules.
...
## 10. Open questions
- Which Linear initiative should permanently own the `planning` plugin work?
