...
- What does the user need?
  ...
  - **User hasn't done the design thinking yet** → hand off to `workflow:architecture` first when available, come back here when the thinking is done
  - **User wants to review an existing doc** → hand off to `workflow:design-review` when available
...
### 2. Requirements
Functional and non-functional. If requirements were gathered with the `planning:requirements` skill, link to that output rather than duplicating. Otherwise, capture them inline. Non-functional requirements need concrete targets: "p99 latency < 200ms" not "fast."

### 3. High-level architecture
Components and how they communicate. Include a Mermaid diagram — delegate to `workflow:c4-diagrams` if the system has more than 4-5 components. Name concrete technology for each component: "Orders Worker (Cloudflare Workers, Hono)" not "Orders Service."

### 4. Data model
Entities, relationships, ownership, storage engine. If the model is detailed, link to the `workflow:data-modeling` output rather than duplicating the full schema here. At minimum, show the core entities and their relationships.
...
### 6. API surface
Main endpoints or operations. If the API is substantial (>5 endpoints), delegate to `codebase:api-design` and link to its output in `.context/architecture/api/`. Here, show only the most important operations with their signatures.
...
|When|Use|
|---|---|
|Need to gather requirements first|`planning:requirements` if installed|
|Need to do architecture thinking|`workflow:architecture` if installed|
|Need a data model|`workflow:data-modeling` if installed|
|API surface is substantial|`codebase:api-design` if installed|
|Need a diagram|`workflow:c4-diagrams`|
|Decisions deserve individual records|`documentation:write-adr`|
|Want to review the finished doc|`workflow:design-review` if installed|
