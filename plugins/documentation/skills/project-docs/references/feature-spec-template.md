...
<!-- Prompt: One or more user stories framing the feature from the perspective of who uses it. The "user" depends on the planning: end user for an app, developer integrating for a library, client for an API, operator for a tool. Format: "As a {who}, I want to {what} so that {why}."
