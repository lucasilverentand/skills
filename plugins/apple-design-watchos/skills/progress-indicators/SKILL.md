...
description: "Progress indicators inform users that your application is actively working, rather than being frozen, while it loads content or executes lengthy processes. Use when designing progress indicators for watchOS, auditing progress indicators against Apple's watchOS guidelines, or when the user says things like \"design progress indicators for Apple Watch\", \"progress indicators on watchOS\", \"how should progress indicators work on Apple Watch\"."
...
## When to use
- User asks about **progress indicators** on watchOS (e.g. `"how do I design progress indicators for Apple Watch"`).
- User is building an Apple Watch UI that needs progress indicators and wants to follow Apple's guidelines.
- User asks to audit or review progress indicators in a watchOS design.
- User mentions progress indicators in the context of an Apple Watch app, game, or interface.
...
The system displays progress indicators in white by default, overlaid on the scene's background. To customize the indicator color, set its tint property.
