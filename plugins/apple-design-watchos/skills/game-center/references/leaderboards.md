...
For games running on iOS, iPadOS, and macOS, use a single image for your leaderboard artwork. For tvOS, provide a series of images that animate when the artwork is in focus. To learn more about focus effects, consult [Focus and selection](focus-and-selection.md). To assist with creating focusable images, download the tvOS template from [Apple Design Resources](https://developer.apple.com/design/resources/#tvos-apps). Use the following specifications when creating leaderboard artwork.
