---
name: id-verifier
description: "ID Verifier allows your iPhone application to read mobile IDs in person without needing external hardware. Use when designing id verifier for watchOS, auditing id verifier against Apple's watchOS guidelines, or when the user says things like \"design id verifier for Apple Watch\", \"id verifier on watchOS\", \"how should id verifier work on Apple Watch\"."
...
## When to use
- User asks about **id verifier** on watchOS (e.g. `"how do I design id verifier for Apple Watch"`).
- User is building an Apple Watch UI that needs id verifier and wants to follow Apple's guidelines.
- User asks to audit or review id verifier in a watchOS design.
- User mentions id verifier in the context of an Apple Watch app, game, or interface.
