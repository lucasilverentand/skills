---
name: activity-views
description: "An activity view, often referred to as a *share sheet*, presents different tasks users can perform within the current context. Use when designing activity views for watchOS, auditing activity views against Apple's watchOS guidelines, or when the user says things like \"design activity views for Apple Watch\", \"activity views on watchOS\", \"how should activity views work on Apple Watch\"."
...
## When to use
- User asks about **activity views** on watchOS (e.g. `"how do I design activity views for Apple Watch"`).
- User is building an Apple Watch UI that needs activity views and wants to follow Apple's guidelines.
- User asks to audit or review activity views in a watchOS design.
- User mentions activity views in the context of an Apple Watch app, game, or interface.
