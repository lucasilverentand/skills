---
name: text-fields
description: "A text field is a rectangular region where users input or modify brief, specific textual data. Use when designing text fields for watchOS, auditing text fields against Apple's watchOS guidelines, or when the user says things like \"design text fields for Apple Watch\", \"text fields on watchOS\", \"how should text fields work on Apple Watch\"."
...
## When to use
- User asks about **text fields** on watchOS (e.g. `"how do I design text fields for Apple Watch"`).
- User is building an Apple Watch UI that needs text fields and wants to follow Apple's guidelines.
- User asks to audit or review text fields in a watchOS design.
- User mentions text fields in the context of an Apple Watch app, game, or interface.
...
Display a text field only when it is essential. Whenever feasible, favor presenting a list of options rather than requiring the user to input text.
