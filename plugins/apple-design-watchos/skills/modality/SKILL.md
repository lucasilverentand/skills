---
name: modality
description: "Modality is a design method where content appears in a standalone, dedicated mode. This prevents interaction with the underlying view and requires an explicit action to dismiss it. Use when designing modality for watchOS, auditing modality against Apple's watchOS guidelines, or when the user says things like \"design modality for Apple Watch\", \"modality on watchOS\", \"how should modality work on Apple Watch\"."
...
## When to use
- User asks about **modality** on watchOS (e.g. `"how do I design modality for Apple Watch"`).
- User is building an Apple Watch UI that needs modality and wants to follow Apple's guidelines.
- User asks to audit or review modality in a watchOS design.
- User mentions modality in the context of an Apple Watch app, game, or interface.
