...
description: "Sign in with Apple offers a rapid and private method for users to access apps and websites, providing a reliable experience they can trust while eliminating the need to recall numerous accounts and…. Use when designing sign in with apple for watchOS, auditing sign in with apple against Apple's watchOS guidelines, or when the user says things like \"design sign in with apple for Apple Watch\", \"sign in with apple on watchOS\", \"how should sign in with apple work on Apple Watch\"."
...
## When to use
- User asks about **sign in with apple** on watchOS (e.g. `"how do I design sign in with apple for Apple Watch"`).
- User is building an Apple Watch UI that needs sign in with apple and wants to follow Apple's guidelines.
- User asks to audit or review sign in with apple in a watchOS design.
- User mentions sign in with apple in the context of an Apple Watch app, game, or interface.
