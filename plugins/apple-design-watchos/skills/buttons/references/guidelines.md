...
Additionally, there are different components that resemble buttons but possess unique appearances and behaviors tailored for specific scenarios, such as [Toggles](toggles.md), [Pop-up buttons](pop-up-buttons.md), and [Segmented controls](segmented-controls.md).
...
- **Avoid applying a similar color to button labels and content layer backgrounds.** If your application already features bright, colorful content in the content layer, it is advisable to use the default monochromatic appearance for button labels. For further details, consult [Liquid Glass color](color.md#Liquid-Glass-color).
...
Inline buttons on watchOS utilize the [capsule](apple:SwiftUI/ButtonBorderShape/capsule) shape. When a button is placed inline with content, it acquires a material effect that contrasts the background, ensuring legibility.

- **Place buttons in the corners using a toolbar.** The system automatically adjusts the time and title to accommodate these toolbar buttons. Additionally, the [Liquid Glass](materials.md#Liquid-Glass) appearance is applied to toolbar buttons, providing a distinct visual separation from the content beneath them.
- **For your app's primary actions, favor buttons that span the screen width.** Full-width buttons are visually superior and easier for users to tap. If two buttons must share the same horizontal space, use identical heights for both and utilize images or concise text titles for their content.
- **Employ toolbar buttons to offer either navigation to related areas or contextual actions for the view’s content.** These buttons grant access to supplementary information or secondary actions pertaining to the view's content.
- **Maintain consistent height when using vertical stacks of one- or two-line text buttons.** To maximize visual consistency, use matching button heights whenever possible.
