---
name: lockups
description: "A lockup integrates multiple distinct views into a single, interactive unit. Use when designing lockups for watchOS, auditing lockups against Apple's watchOS guidelines, or when the user says things like \"design lockups for Apple Watch\", \"lockups on watchOS\", \"how should lockups work on Apple Watch\"."
...
## When to use
- User asks about **lockups** on watchOS (e.g. `"how do I design lockups for Apple Watch"`).
- User is building an Apple Watch UI that needs lockups and wants to follow Apple's guidelines.
- User asks to audit or review lockups in a watchOS design.
- User mentions lockups in the context of an Apple Watch app, game, or interface.
