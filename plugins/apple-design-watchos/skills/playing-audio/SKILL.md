---
name: playing-audio
description: "Users anticipate sophisticated audio experiences that dynamically adapt based on the device's current context. Use when designing playing audio for watchOS, auditing playing audio against Apple's watchOS guidelines, or when the user says things like \"design playing audio for Apple Watch\", \"playing audio on watchOS\", \"how should playing audio work on Apple Watch\"."
...
## When to use
- User asks about **playing audio** on watchOS (e.g. `"how do I design playing audio for Apple Watch"`).
- User is building an Apple Watch UI that needs playing audio and wants to follow Apple's guidelines.
- User asks to audit or review playing audio in a watchOS design.
- User mentions playing audio in the context of an Apple Watch app, game, or interface.
