---
name: color
description: "Thoughtful color usage serves multiple purposes: enhancing communication, establishing brand identity, ensuring visual continuity, conveying status and feedback, and improving information comprehen…. Use when designing color for watchOS, auditing color against Apple's watchOS guidelines, or when the user says things like \"design color for Apple Watch\", \"color on watchOS\", \"how should color work on Apple Watch\"."
...
## When to use
- User asks about **color** on watchOS (e.g. `"how do I design color for Apple Watch"`).
- User is building an Apple Watch UI that needs color and wants to follow Apple's guidelines.
- User asks to audit or review color in a watchOS design.
- User mentions color in the context of an Apple Watch app, game, or interface.
