---
name: page-controls
description: "A page control presents a horizontal row of indicator images, with each image representing a specific page in a linear list. Use when designing page controls for watchOS, auditing page controls against Apple's watchOS guidelines, or when the user says things like \"design page controls for Apple Watch\", \"page controls on watchOS\", \"how should page controls work on Apple Watch\"."
...
## When to use
- User asks about **page controls** on watchOS (e.g. `"how do I design page controls for Apple Watch"`).
- User is building an Apple Watch UI that needs page controls and wants to follow Apple's guidelines.
- User asks to audit or review page controls in a watchOS design.
- User mentions page controls in the context of an Apple Watch app, game, or interface.
...
In watchOS, page controls appear at the bottom of the screen for horizontal pagination, or adjacent to the Digital Crown when implementing a vertical [tab view](components/layout-and-organization/tab-views.md). When employing vertical tab views, the page indicator informs users of their location within both the current page and the collection of pages. The page control manages transitions between scrolling through a page's content and navigating to different pages.
