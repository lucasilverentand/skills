---
name: status-bars
description: "The status bar occupies the upper edge of the display and provides indicators regarding the device's current operational state, such as the time, cellular carrier, and battery level. Use when designing status bars for watchOS, auditing status bars against Apple's watchOS guidelines, or when the user says things like \"design status bars for Apple Watch\", \"status bars on watchOS\", \"how should status bars work on Apple Watch\"."
...
## When to use
- User asks about **status bars** on watchOS (e.g. `"how do I design status bars for Apple Watch"`).
- User is building an Apple Watch UI that needs status bars and wants to follow Apple's guidelines.
- User asks to audit or review status bars in a watchOS design.
- User mentions status bars in the context of an Apple Watch app, game, or interface.
