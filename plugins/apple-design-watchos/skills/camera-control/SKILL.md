---
name: camera-control
description: "The Camera Control grants immediate access to your application's camera functionality. Use when designing camera control for watchOS, auditing camera control against Apple's watchOS guidelines, or when the user says things like \"design camera control for Apple Watch\", \"camera control on watchOS\", \"how should camera control work on Apple Watch\"."
...
## When to use
- User asks about **camera control** on watchOS (e.g. `"how do I design camera control for Apple Watch"`).
- User is building an Apple Watch UI that needs camera control and wants to follow Apple's guidelines.
- User asks to audit or review camera control in a watchOS design.
- User mentions camera control in the context of an Apple Watch app, game, or interface.
