---
name: nfc
description: "Near-field communication (NFC) enables devices located within a few centimeters of one another to exchange data wirelessly. Use when designing nfc for watchOS, auditing nfc against Apple's watchOS guidelines, or when the user says things like \"design nfc for Apple Watch\", \"nfc on watchOS\", \"how should nfc work on Apple Watch\"."
...
## When to use
- User asks about **nfc** on watchOS (e.g. `"how do I design nfc for Apple Watch"`).
- User is building an Apple Watch UI that needs nfc and wants to follow Apple's guidelines.
- User asks to audit or review nfc in a watchOS design.
- User mentions nfc in the context of an Apple Watch app, game, or interface.
