...
- What are you doing?
  ...
  - **Improving an existing skill** -> what needs work?
    ...
    - **Automatable tasks would benefit from a tool** -> follow "Adding a tool" below
...
3. Write the decision tree — see `references/decision-trees.md`
4. Write conventions — the rules the agent must follow every time
5. Optionally add tools in `tools/` if the skill has automatable tasks (see "Adding a tool") — many skills work fine without any tools
