...
description: "A rating indicator conveys a ranking level using a sequence of horizontally arranged graphical symbols, which default to stars. Use when designing rating indicators for macOS, auditing rating indicators against Apple's macOS guidelines, or when the user says things like \"design rating indicators for Mac\", \"rating indicators on macOS\", \"how should rating indicators work on Mac\"."
...
## When to use
- User asks about **rating indicators** on macOS (e.g. `"how do I design rating indicators for Mac"`).
- User is building a Mac UI that needs rating indicators and wants to follow Apple's guidelines.
- User asks to audit or review rating indicators in a macOS design.
- User mentions rating indicators in the context of a Mac app, game, or interface.
