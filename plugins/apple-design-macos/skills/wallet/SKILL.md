---
name: wallet
description: "Wallet enables users to securely store items on iPhone and Apple Watch, such as credit and debit cards, driver’s licenses or state IDs, transit passes, event tickets, keys, and other items. Use when designing wallet for macOS, auditing wallet against Apple's macOS guidelines, or when the user says things like \"design wallet for Mac\", \"wallet on macOS\", \"how should wallet work on Mac\"."
...
## When to use
- User asks about **wallet** on macOS (e.g. `"how do I design wallet for Mac"`).
- User is building a Mac UI that needs wallet and wants to follow Apple's guidelines.
- User asks to audit or review wallet in a macOS design.
- User mentions wallet in the context of a Mac app, game, or interface.
