---
name: camera-control
description: "The Camera Control grants immediate access to your application's camera functionality. Use when designing camera control for macOS, auditing camera control against Apple's macOS guidelines, or when the user says things like \"design camera control for Mac\", \"camera control on macOS\", \"how should camera control work on Mac\"."
...
## When to use
- User asks about **camera control** on macOS (e.g. `"how do I design camera control for Mac"`).
- User is building a Mac UI that needs camera control and wants to follow Apple's guidelines.
- User asks to audit or review camera control in a macOS design.
- User mentions camera control in the context of a Mac app, game, or interface.
