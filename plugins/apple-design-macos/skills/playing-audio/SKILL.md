---
name: playing-audio
description: "Users anticipate sophisticated audio experiences that dynamically adapt based on the device's current context. Use when designing playing audio for macOS, auditing playing audio against Apple's macOS guidelines, or when the user says things like \"design playing audio for Mac\", \"playing audio on macOS\", \"how should playing audio work on Mac\"."
...
## When to use
- User asks about **playing audio** on macOS (e.g. `"how do I design playing audio for Mac"`).
- User is building a Mac UI that needs playing audio and wants to follow Apple's guidelines.
- User asks to audit or review playing audio in a macOS design.
- User mentions playing audio in the context of a Mac app, game, or interface.
