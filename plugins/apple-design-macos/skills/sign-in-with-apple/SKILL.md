...
description: "Sign in with Apple offers a rapid and private method for users to access apps and websites, providing a reliable experience they can trust while eliminating the need to recall numerous accounts and…. Use when designing sign in with apple for macOS, auditing sign in with apple against Apple's macOS guidelines, or when the user says things like \"design sign in with apple for Mac\", \"sign in with apple on macOS\", \"how should sign in with apple work on Mac\"."
...
## When to use
- User asks about **sign in with apple** on macOS (e.g. `"how do I design sign in with apple for Mac"`).
- User is building a Mac UI that needs sign in with apple and wants to follow Apple's guidelines.
- User asks to audit or review sign in with apple in a macOS design.
- User mentions sign in with apple in the context of a Mac app, game, or interface.
