---
name: nfc
description: "Near-field communication (NFC) enables devices located within a few centimeters of one another to exchange data wirelessly. Use when designing nfc for macOS, auditing nfc against Apple's macOS guidelines, or when the user says things like \"design nfc for Mac\", \"nfc on macOS\", \"how should nfc work on Mac\"."
...
## When to use
- User asks about **nfc** on macOS (e.g. `"how do I design nfc for Mac"`).
- User is building a Mac UI that needs nfc and wants to follow Apple's guidelines.
- User asks to audit or review nfc in a macOS design.
- User mentions nfc in the context of a Mac app, game, or interface.
