---
name: search-fields
description: "A search field allows users to locate specific content within a collection using entered terms. Use when designing search fields for macOS, auditing search fields against Apple's macOS guidelines, or when the user says things like \"design search fields for Mac\", \"search fields on macOS\", \"how should search fields work on Mac\"."
...
## When to use
- User asks about **search fields** on macOS (e.g. `"how do I design search fields for Mac"`).
- User is building a Mac UI that needs search fields and wants to follow Apple's guidelines.
- User asks to audit or review search fields in a macOS design.
- User mentions search fields in the context of a Mac app, game, or interface.
