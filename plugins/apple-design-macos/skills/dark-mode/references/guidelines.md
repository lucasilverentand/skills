...
#### Icons and images
The system leverages [SF Symbols](sf-symbols.md) (which automatically adjust for Dark Mode) alongside full-color images optimized for both light and dark themes.

- **Utilize SF Symbols whenever feasible.** These symbols render effectively in either appearance mode when dynamic colors are applied for tinting or vibrancy is added. Refer to [Color](color.md) for detailed guidance.
...
When users select the graphite accent color in General settings, macOS uses the current desktop picture to influence window backgrounds. This effect, known as *desktop tinting*, provides a subtle visual blending that helps windows harmonize with their surrounding content.

**Incorporate transparency into custom component backgrounds when appropriate.** Transparency enables your components to inherit color from the window background while desktop tinting is active, maintaining visual coherence even if the desktop picture changes. To successfully achieve this harmony, apply transparency exclusively to a custom component that possesses a visible background or bezel, and only when the component is in a neutral state (i.e., not utilizing color). Avoid adding transparency when the component is displaying color, as this can cause its hue to fluctuate if the window background shifts or the desktop picture changes.
