---
name: id-verifier
description: "ID Verifier allows your iPhone application to read mobile IDs in person without needing external hardware. Use when designing id verifier for macOS, auditing id verifier against Apple's macOS guidelines, or when the user says things like \"design id verifier for Mac\", \"id verifier on macOS\", \"how should id verifier work on Mac\"."
...
## When to use
- User asks about **id verifier** on macOS (e.g. `"how do I design id verifier for Mac"`).
- User is building a Mac UI that needs id verifier and wants to follow Apple's guidelines.
- User asks to audit or review id verifier in a macOS design.
- User mentions id verifier in the context of a Mac app, game, or interface.
