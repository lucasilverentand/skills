---
name: wallet
description: "Wallet enables users to securely store items on iPhone and Apple Watch, such as credit and debit cards, driver’s licenses or state IDs, transit passes, event tickets, keys, and other items. Use when designing wallet for tvOS, auditing wallet against Apple's tvOS guidelines, or when the user says things like \"design wallet for Apple TV\", \"wallet on tvOS\", \"how should wallet work on Apple TV\"."
...
## When to use
- User asks about **wallet** on tvOS (e.g. `"how do I design wallet for Apple TV"`).
- User is building an Apple TV UI that needs wallet and wants to follow Apple's guidelines.
- User asks to audit or review wallet in a tvOS design.
- User mentions wallet in the context of an Apple TV app, game, or interface.
