...
description: "On devices lacking a physical keyboard, the operating system provides several virtual keyboard options for data input. Use when designing virtual keyboards for tvOS, auditing virtual keyboards against Apple's tvOS guidelines, or when the user says things like \"design virtual keyboards for Apple TV\", \"virtual keyboards on tvOS\", \"how should virtual keyboards work on Apple TV\"."
...
## When to use
- User asks about **virtual keyboards** on tvOS (e.g. `"how do I design virtual keyboards for Apple TV"`).
- User is building an Apple TV UI that needs virtual keyboards and wants to follow Apple's guidelines.
- User asks to audit or review virtual keyboards in a tvOS design.
- User mentions virtual keyboards in the context of an Apple TV app, game, or interface.
...
tvOS presents a linear virtual keyboard when users select a text field using the Siri Remote.

> **Note**
> When devices other than the Siri Remote are utilized, a grid keyboard screen appears, and the content layout automatically adjusts to suit the keyboard.

When a digit entry view is activated, tvOS displays a specialized keyboard for numerical input. For further instructions, consult [Digit entry views](digit-entry-views.md).
