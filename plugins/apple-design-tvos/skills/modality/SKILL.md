---
name: modality
description: "Modality is a design method where content appears in a standalone, dedicated mode. This prevents interaction with the underlying view and requires an explicit action to dismiss it. Use when designing modality for tvOS, auditing modality against Apple's tvOS guidelines, or when the user says things like \"design modality for Apple TV\", \"modality on tvOS\", \"how should modality work on Apple TV\"."
...
## When to use
- User asks about **modality** on tvOS (e.g. `"how do I design modality for Apple TV"`).
- User is building an Apple TV UI that needs modality and wants to follow Apple's guidelines.
- User asks to audit or review modality in a tvOS design.
- User mentions modality in the context of an Apple TV app, game, or interface.
