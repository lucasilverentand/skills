---
name: playing-audio
description: "Users anticipate sophisticated audio experiences that dynamically adapt based on the device's current context. Use when designing playing audio for tvOS, auditing playing audio against Apple's tvOS guidelines, or when the user says things like \"design playing audio for Apple TV\", \"playing audio on tvOS\", \"how should playing audio work on Apple TV\"."
...
## When to use
- User asks about **playing audio** on tvOS (e.g. `"how do I design playing audio for Apple TV"`).
- User is building an Apple TV UI that needs playing audio and wants to follow Apple's guidelines.
- User asks to audit or review playing audio in a tvOS design.
- User mentions playing audio in the context of an Apple TV app, game, or interface.
