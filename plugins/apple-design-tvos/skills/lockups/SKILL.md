---
name: lockups
description: "A lockup integrates multiple distinct views into a single, interactive unit. Use when designing lockups for tvOS, auditing lockups against Apple's tvOS guidelines, or when the user says things like \"design lockups for Apple TV\", \"lockups on tvOS\", \"how should lockups work on Apple TV\"."
...
## When to use
- User asks about **lockups** on tvOS (e.g. `"how do I design lockups for Apple TV"`).
- User is building an Apple TV UI that needs lockups and wants to follow Apple's guidelines.
- User asks to audit or review lockups in a tvOS design.
- User mentions lockups in the context of an Apple TV app, game, or interface.
