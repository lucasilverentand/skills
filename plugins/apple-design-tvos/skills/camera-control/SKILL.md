---
name: camera-control
description: "The Camera Control grants immediate access to your application's camera functionality. Use when designing camera control for tvOS, auditing camera control against Apple's tvOS guidelines, or when the user says things like \"design camera control for Apple TV\", \"camera control on tvOS\", \"how should camera control work on Apple TV\"."
...
## When to use
- User asks about **camera control** on tvOS (e.g. `"how do I design camera control for Apple TV"`).
- User is building an Apple TV UI that needs camera control and wants to follow Apple's guidelines.
- User asks to audit or review camera control in a tvOS design.
- User mentions camera control in the context of an Apple TV app, game, or interface.
