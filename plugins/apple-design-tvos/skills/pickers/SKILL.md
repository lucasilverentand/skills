---
name: pickers
description: "A picker presents one or more scrollable lists containing unique values from which users can select. Use when designing pickers for tvOS, auditing pickers against Apple's tvOS guidelines, or when the user says things like \"design pickers for Apple TV\", \"pickers on tvOS\", \"how should pickers work on Apple TV\"."
...
## When to use
- User asks about **pickers** on tvOS (e.g. `"how do I design pickers for Apple TV"`).
- User is building an Apple TV UI that needs pickers and wants to follow Apple's guidelines.
- User asks to audit or review pickers in a tvOS design.
- User mentions pickers in the context of an Apple TV app, game, or interface.
...
Pickers can be implemented in tvOS using SwiftUI. Developers seeking guidance should consult [Picker](apple:SwiftUI/Picker).
