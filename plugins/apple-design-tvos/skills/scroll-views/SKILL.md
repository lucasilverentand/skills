---
name: scroll-views
description: "A scroll view enables users to examine content that exceeds the display boundaries by shifting the content along a vertical or horizontal axis. Use when designing scroll views for tvOS, auditing scroll views against Apple's tvOS guidelines, or when the user says things like \"design scroll views for Apple TV\", \"scroll views on tvOS\", \"how should scroll views work on Apple TV\"."
...
## When to use
- User asks about **scroll views** on tvOS (e.g. `"how do I design scroll views for Apple TV"`).
- User is building an Apple TV UI that needs scroll views and wants to follow Apple's guidelines.
- User asks to audit or review scroll views in a tvOS design.
- User mentions scroll views in the context of an Apple TV app, game, or interface.
...
Views on tvOS support scrolling, but they are not treated as distinct objects requiring dedicated scroll indicators. Instead, when the content exceeds the screen dimensions, the system automatically scrolls the interface to ensure focused items remain visible.
