---
name: status-bars
description: "The status bar occupies the upper edge of the display and provides indicators regarding the device's current operational state, such as the time, cellular carrier, and battery level. Use when designing status bars for tvOS, auditing status bars against Apple's tvOS guidelines, or when the user says things like \"design status bars for Apple TV\", \"status bars on tvOS\", \"how should status bars work on Apple TV\"."
...
## When to use
- User asks about **status bars** on tvOS (e.g. `"how do I design status bars for Apple TV"`).
- User is building an Apple TV UI that needs status bars and wants to follow Apple's guidelines.
- User asks to audit or review status bars in a tvOS design.
- User mentions status bars in the context of an Apple TV app, game, or interface.
