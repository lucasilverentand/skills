---
name: gestures
description: "A gesture is a physical motion that a user employs to directly influence an object within an application or game on their device. Use when designing gestures for tvOS, auditing gestures against Apple's tvOS guidelines, or when the user says things like \"design gestures for Apple TV\", \"gestures on tvOS\", \"how should gestures work on Apple TV\"."
...
## When to use
- User asks about **gestures** on tvOS (e.g. `"how do I design gestures for Apple TV"`).
- User is building an Apple TV UI that needs gestures and wants to follow Apple's guidelines.
- User asks to audit or review gestures in a tvOS design.
- User mentions gestures in the context of an Apple TV app, game, or interface.

Based on the hardware being used, users can execute gestures via a touchscreen, through air input, or using different peripherals such as a trackpad, mouse, remote, or game controller that includes a touch surface.
...
Users anticipate navigating tvOS applications and games using [Standard gestures](#Standard-gestures) when interacting with a compatible remote, Siri Remote, or [Game controls](game-controls.md) that incorporates a touch surface. Consult [Remotes](remotes.md) for guidance.
