---
name: drag-and-drop
description: "Drag and drop allows users to relocate or duplicate selected items, such as photos, text, or other content, by moving the selection from one location to another. Use when designing drag and drop for tvOS, auditing drag and drop against Apple's tvOS guidelines, or when the user says things like \"design drag and drop for Apple TV\", \"drag and drop on tvOS\", \"how should drag and drop work on Apple TV\"."
...
## When to use
- User asks about **drag and drop** on tvOS (e.g. `"how do I design drag and drop for Apple TV"`).
- User is building an Apple TV UI that needs drag and drop and wants to follow Apple's guidelines.
- User asks to audit or review drag and drop in a tvOS design.
- User mentions drag and drop in the context of an Apple TV app, game, or interface.
