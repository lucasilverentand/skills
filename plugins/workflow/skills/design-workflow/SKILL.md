...
This plugin owns the core system design workflow skills: `architecture`, `data-modeling`, `design-review`, and `c4-diagrams`. For full end-to-end work, compose with companion plugin skills when they are installed: `planning:requirements`, `codebase:api-design`, `documentation:write-design-doc`, and `documentation:write-adr`.
...
**What to do:** If `planning:requirements` is installed, follow it. Otherwise gather the minimum requirements inline. The output is a structured requirements doc in `.context/architecture/requirements/`. The key deliverables are: functional requirements (MoSCoW-prioritized, numbered), NFR targets with concrete numbers, constraints, and open questions.
...
**Diagrams:** If the system has >4 components or the user wants a visual, produce a C4 diagram via `workflow:c4-diagrams` when available. A Container-level diagram is the most useful default.
