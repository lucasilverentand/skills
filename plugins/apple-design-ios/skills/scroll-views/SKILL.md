---
name: scroll-views
description: "A scroll view enables users to examine content that exceeds the display boundaries by shifting the content along a vertical or horizontal axis. Use when designing scroll views for iOS and iPadOS, auditing scroll views against Apple's iOS and iPadOS guidelines, or when the user says things like \"design scroll views for iPhone\", \"scroll views on iOS and iPadOS\", \"how should scroll views work on iPhone\"."
...
## When to use
- User asks about **scroll views** on iOS and iPadOS (e.g. `"how do I design scroll views for iPhone"`).
- User is building an iPhone UI that needs scroll views and wants to follow Apple's guidelines.
- User asks to audit or review scroll views in an iOS and iPadOS design.
- User mentions scroll views in the context of an iPhone app, game, or interface.
...
**Consider displaying a page control when the scroll view operates in discrete, page-by-page mode.** [Page controls](page-controls.md) indicate the total number of pages, screens, or content segments available and specify which segment is currently displayed. For instance, the Weather app utilizes a page control to track transitions between users' saved locations. If both a page control and a scroll view are present, ensure that the scrolling indicator does not appear on the same axis to prevent user confusion caused by redundant controls.
