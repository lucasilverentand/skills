---
name: status-bars
description: "The status bar occupies the upper edge of the display and provides indicators regarding the device's current operational state, such as the time, cellular carrier, and battery level. Use when designing status bars for iOS and iPadOS, auditing status bars against Apple's iOS and iPadOS guidelines, or when the user says things like \"design status bars for iPhone\", \"status bars on iOS and iPadOS\", \"how should status bars work on iPhone\"."
...
## When to use
- User asks about **status bars** on iOS and iPadOS (e.g. `"how do I design status bars for iPhone"`).
- User is building an iPhone UI that needs status bars and wants to follow Apple's guidelines.
- User asks to audit or review status bars in an iOS and iPadOS design.
- User mentions status bars in the context of an iPhone app, game, or interface.
