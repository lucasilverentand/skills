...
description: "A segmented control presents as a linear arrangement of two or more segments, each acting as a button. Use when designing segmented controls for iOS and iPadOS, auditing segmented controls against Apple's iOS and iPadOS guidelines, or when the user says things like \"design segmented controls for iPhone\", \"segmented controls on iOS and iPadOS\", \"how should segmented controls work on iPhone\"."
...
## When to use
- User asks about **segmented controls** on iOS and iPadOS (e.g. `"how do I design segmented controls for iPhone"`).
- User is building an iPhone UI that needs segmented controls and wants to follow Apple's guidelines.
- User asks to audit or review segmented controls in an iOS and iPadOS design.
- User mentions segmented controls in the context of an iPhone app, game, or interface.
...
A segmented control is appropriate when switching between closely related subviews. This component allows users to quickly navigate between linked views; for example, the segmented control within Calendar’s New Event sheet facilitates switching between creating a new event and a new reminder. However, if the application sections are entirely separate, you must utilize [Tab bars](tab-bars.md) instead.
