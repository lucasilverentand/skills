---
name: text-fields
description: "A text field is a rectangular region where users input or modify brief, specific textual data. Use when designing text fields for iOS and iPadOS, auditing text fields against Apple's iOS and iPadOS guidelines, or when the user says things like \"design text fields for iPhone\", \"text fields on iOS and iPadOS\", \"how should text fields work on iPhone\"."
...
## When to use
- User asks about **text fields** on iOS and iPadOS (e.g. `"how do I design text fields for iPhone"`).
- User is building an iPhone UI that needs text fields and wants to follow Apple's guidelines.
- User asks to audit or review text fields in an iOS and iPadOS design.
- User mentions text fields in the context of an iPhone app, game, or interface.
...
- **Include a Clear button at the trailing edge of a text field to assist users in removing their input.** When this element is present, tapping it allows the user to wipe the text field's contents without needing continuous use of the Delete key.
- **Employ images and buttons to enhance the clarity and functionality of text fields.** You may display custom images at either end of a text field, or you can incorporate a system-provided button, such as the Bookmarks feature. Generally, designate the leading end of a text field to convey its purpose and the trailing end for supplementary functions like bookmarking.
