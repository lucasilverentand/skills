---
name: sheets
description: "A sheet enables users to execute a focused task that is contextually linked to their current view. Use when designing sheets for visionOS, auditing sheets against Apple's visionOS guidelines, or when the user says things like \"design sheets for Apple Vision Pro\", \"sheets on visionOS\", \"how should sheets work on Apple Vision Pro\"."
...
## When to use
- User asks about **sheets** on visionOS (e.g. `"how do I design sheets for Apple Vision Pro"`).
- User is building an Apple Vision Pro UI that needs sheets and wants to follow Apple's guidelines.
- User asks to audit or review sheets in a visionOS design.
- User mentions sheets in the context of an Apple Vision Pro app, game, or interface.
...
- **Do not allow a sheet to appear emerging from the bottom edge of a window.** To optimize visibility for users, it is recommended that you center the sheet within their [Field of view](spatial-layout.md#Field-of-view).
