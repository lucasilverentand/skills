---
name: wallet
description: "Wallet enables users to securely store items on iPhone and Apple Watch, such as credit and debit cards, driver’s licenses or state IDs, transit passes, event tickets, keys, and other items. Use when designing wallet for visionOS, auditing wallet against Apple's visionOS guidelines, or when the user says things like \"design wallet for Apple Vision Pro\", \"wallet on visionOS\", \"how should wallet work on Apple Vision Pro\"."
...
## When to use
- User asks about **wallet** on visionOS (e.g. `"how do I design wallet for Apple Vision Pro"`).
- User is building an Apple Vision Pro UI that needs wallet and wants to follow Apple's guidelines.
- User asks to audit or review wallet in a visionOS design.
- User mentions wallet in the context of an Apple Vision Pro app, game, or interface.
