---
name: search-fields
description: "A search field allows users to locate specific content within a collection using entered terms. Use when designing search fields for visionOS, auditing search fields against Apple's visionOS guidelines, or when the user says things like \"design search fields for Apple Vision Pro\", \"search fields on visionOS\", \"how should search fields work on Apple Vision Pro\"."
...
## When to use
- User asks about **search fields** on visionOS (e.g. `"how do I design search fields for Apple Vision Pro"`).
- User is building an Apple Vision Pro UI that needs search fields and wants to follow Apple's guidelines.
- User asks to audit or review search fields in a visionOS design.
- User mentions search fields in the context of an Apple Vision Pro app, game, or interface.
