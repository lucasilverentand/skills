---
name: status-bars
description: "The status bar occupies the upper edge of the display and provides indicators regarding the device's current operational state, such as the time, cellular carrier, and battery level. Use when designing status bars for visionOS, auditing status bars against Apple's visionOS guidelines, or when the user says things like \"design status bars for Apple Vision Pro\", \"status bars on visionOS\", \"how should status bars work on Apple Vision Pro\"."
...
## When to use
- User asks about **status bars** on visionOS (e.g. `"how do I design status bars for Apple Vision Pro"`).
- User is building an Apple Vision Pro UI that needs status bars and wants to follow Apple's guidelines.
- User asks to audit or review status bars in a visionOS design.
- User mentions status bars in the context of an Apple Vision Pro app, game, or interface.
