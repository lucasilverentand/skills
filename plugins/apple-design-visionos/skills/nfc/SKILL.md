---
name: nfc
description: "Near-field communication (NFC) enables devices located within a few centimeters of one another to exchange data wirelessly. Use when designing nfc for visionOS, auditing nfc against Apple's visionOS guidelines, or when the user says things like \"design nfc for Apple Vision Pro\", \"nfc on visionOS\", \"how should nfc work on Apple Vision Pro\"."
...
## When to use
- User asks about **nfc** on visionOS (e.g. `"how do I design nfc for Apple Vision Pro"`).
- User is building an Apple Vision Pro UI that needs nfc and wants to follow Apple's guidelines.
- User asks to audit or review nfc in a visionOS design.
- User mentions nfc in the context of an Apple Vision Pro app, game, or interface.
