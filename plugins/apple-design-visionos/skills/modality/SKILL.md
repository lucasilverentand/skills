---
name: modality
description: "Modality is a design method where content appears in a standalone, dedicated mode. This prevents interaction with the underlying view and requires an explicit action to dismiss it. Use when designing modality for visionOS, auditing modality against Apple's visionOS guidelines, or when the user says things like \"design modality for Apple Vision Pro\", \"modality on visionOS\", \"how should modality work on Apple Vision Pro\"."
...
## When to use
- User asks about **modality** on visionOS (e.g. `"how do I design modality for Apple Vision Pro"`).
- User is building an Apple Vision Pro UI that needs modality and wants to follow Apple's guidelines.
- User asks to audit or review modality in a visionOS design.
- User mentions modality in the context of an Apple Vision Pro app, game, or interface.
