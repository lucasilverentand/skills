---
name: activity-views
description: "An activity view, often referred to as a *share sheet*, presents different tasks users can perform within the current context. Use when designing activity views for visionOS, auditing activity views against Apple's visionOS guidelines, or when the user says things like \"design activity views for Apple Vision Pro\", \"activity views on visionOS\", \"how should activity views work on Apple Vision Pro\"."
...
## When to use
- User asks about **activity views** on visionOS (e.g. `"how do I design activity views for Apple Vision Pro"`).
- User is building an Apple Vision Pro UI that needs activity views and wants to follow Apple's guidelines.
- User asks to audit or review activity views in a visionOS design.
- User mentions activity views in the context of an Apple Vision Pro app, game, or interface.
