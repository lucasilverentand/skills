---
name: drag-and-drop
description: "Drag and drop allows users to relocate or duplicate selected items, such as photos, text, or other content, by moving the selection from one location to another. Use when designing drag and drop for visionOS, auditing drag and drop against Apple's visionOS guidelines, or when the user says things like \"design drag and drop for Apple Vision Pro\", \"drag and drop on visionOS\", \"how should drag and drop work on Apple Vision Pro\"."
...
## When to use
- User asks about **drag and drop** on visionOS (e.g. `"how do I design drag and drop for Apple Vision Pro"`).
- User is building an Apple Vision Pro UI that needs drag and drop and wants to follow Apple's guidelines.
- User asks to audit or review drag and drop in a visionOS design.
- User mentions drag and drop in the context of an Apple Vision Pro app, game, or interface.
