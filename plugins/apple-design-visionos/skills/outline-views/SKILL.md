---
name: outline-views
description: "An outline view displays structured data hierarchically within a scrollable sequence of cells arranged in columns and rows. Use when designing outline views for visionOS, auditing outline views against Apple's visionOS guidelines, or when the user says things like \"design outline views for Apple Vision Pro\", \"outline views on visionOS\", \"how should outline views work on Apple Vision Pro\"."
...
## When to use
- User asks about **outline views** on visionOS (e.g. `"how do I design outline views for Apple Vision Pro"`).
- User is building an Apple Vision Pro UI that needs outline views and wants to follow Apple's guidelines.
- User asks to audit or review outline views in a visionOS design.
- User mentions outline views in the context of an Apple Vision Pro app, game, or interface.
