...
- **Use an action sheet, rather than an alert, when presenting choices resulting from a deliberate user action.** For example, if users cancel changes to an edited Mail message, the action sheet presents three options: discard the edits (or the entire draft), save the draft, or return to editing. While an alert can be used to confirm or cancel actions with destructive consequences, it does not offer additional choices regarding the action. For further guidance, consult [Action sheets](action-sheets.md).
- **When possible, prevent alerts from scrolling.** Even if an alert might scroll due to large text size, you must minimize the potential for scrolling by keeping alert titles succinct and including only a brief message when required.
