...
##### Double tap
In watchOS 11 and later, users can employ the double-tap gesture to navigate through lists, scroll views, and transition between vertical tab views. Furthermore, you have the option to designate a toggle or button as the primary action within your application, widget, or Live Activity when it is displayed in the Smart Stack. When double-tapping a view that includes a primary action, the control is highlighted before executing the action. The system also supports double-tap for custom actions provided within [Notifications](notifications.md), where it triggers the first non-destructive action in the notification.
...
- **Select the button that users most frequently utilize as the primary action in a view.** Double-tapping is beneficial in a static (non-scrolling) view if it executes the action that users perform most often. For instance, in a media controls screen, you might assign the primary action to the play/pause control. For developer guidance, consult [handGestureShortcut(_:isEnabled:)](apple:SwiftUI/View/handGestureShortcut(_:isEnabled:)) and [primaryAction](apple:SwiftUI/HandGestureShortcut/primaryAction).
