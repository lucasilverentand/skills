...
The Digital Crown on Apple Watch Series 4 and later models provides haptic feedback, offering users a more tactile interaction while scrolling through content. By default, the system delivers linear haptic detents that users perceive when rotating the Digital Crown. Furthermore, certain controls, such as `table views`, provide these detents as new content scrolls into view. For developer reference, consult [WKHapticType](apple:WatchKit/WKHapticType).

watchOS has defined a specific collection of haptics, each conveying a unique meaning to the user.
