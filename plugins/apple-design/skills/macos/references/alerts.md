...
- macOS alerts have the option to add suppression controls [Checkboxes](toggles.md#Checkboxes) and a link to **Help buttons**.
...
macOS automatically displays your application icon in an alert, but you retain the ability to supply an alternative symbol or icon. Additionally, macOS enables you to:

- Configure alerts to repeat, allowing users to suppress subsequent occurrences of the same alert.
- Append a custom view if additional information is necessary (refer to [accessoryView](apple:AppKit/NSAlert/accessoryView) for developer guidance).
- Include a Help button that launches your help documentation (see **Help buttons**).

**Use a caution symbol sparingly.** Frequent use of a caution symbol, such as `exclamationmark.triangle`, diminishes its importance. Employ this symbol only when extra user attention is genuinely required, such as confirming an action that may result in unexpected data loss. Do not use the symbol for tasks whose sole function is overwriting or removal, like saving or emptying trash.
