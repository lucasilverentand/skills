...
Examples: `development/committing`, `maintenance/branching`, `collaboration/coauthoring`, `security/audit`
...
Examples: `development` (parent routing to children), `collaboration/developer-docs` (composes `style` + `knowledge`), `delivery/clean`
...
2. **Cross-skill references** — the skill points to another skill for a specific concern (e.g., "for tone -> see `collaboration/coauthoring`")
