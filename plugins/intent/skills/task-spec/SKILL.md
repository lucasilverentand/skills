...
- What is the user trying to start?
  ...
  - **Linear or GitHub issue creation** -> run "Spec intake", then write ticket-ready issue text. For Linear, hand off to `intent:linear-issues` when available. Use the relevant issue-creation workflow only after the brief is coherent.
...
- **If the user asked for an issue** -> create or update the issue using the relevant Linear/GitHub workflow, preserving the brief structure. For Linear, use `intent:linear-issues` when available.
