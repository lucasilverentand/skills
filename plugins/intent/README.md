# Intent
Intent and problem-understanding workflows for requirements, task shaping, Linear planning, project documents, issue hygiene, and turning vague goals into traceable work.

## Skills
- `intent:create-doc-template`
- `intent:create-planner-project`
- `intent:estimate-issue-complexity`
- `intent:issue-authoring`
- `intent:linear-issues`
- `intent:pickup-work`
...
- `intent:requirements`
- `intent:task-spec`
- `intent:tidy-linear-project`

Codex and Claude Code also expose command shims for this plugin: `/intent:pickup-work`, `/intent:tidy-linear-project`. Prefer the portable skills above for automatic loading.
...
/plugin install intent@skills-of-luca
```
...
3. Install `intent` (Required or Optional)
...
This plugin owns its skill source under `plugins/intent/skills/`. Edit those files directly, then run `bun run marketplace:write` to refresh generated manifests and marketplaces.
